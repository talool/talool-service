...
public final class QueryHelper {
  ...
  private static final String DEAL_OFFER_BASIC_STATS =
      "select dof.deal_offer_id as dealOfferId,count(distinct d.merchant_id) as totalMerchants,count(d.deal_id) as totalDeals "
          + "from merchant as m,deal as d,deal_offer as dof " + "where d.deal_offer_id in (select deal_offer_id from deal_offer ) "
  ...
  private static final String DEAL_OFFER_SUMMARY =
      "select DISTINCT o.deal_offer_id as offerId, o.merchant_id as merchantId, o.title as title, o.summary as summary,  %%o.properties AS properties, "
  ...
  private static final String MERCHANT_CODE_SUMMARY = "SELECT g.code_group_title AS name, g.code_group_notes AS email, c.code AS code, "
      + "(SELECT count(distinct deal_offer_purchase_id) FROM deal_offer_purchase " + "WHERE properties->'merchant_code' = c.code ) AS purchaseCount "
      + "FROM merchant_code AS c, merchant_code_group AS g " + "WHERE c.merchant_code_group_id=g.merchant_code_group_id "
      + "AND g.merchant_id = :merchantId ";
  ...
  private static final String PUBLISHER_FUNDRAISER_SUMMARY =
      "SELECT m.merchant_id AS merchantId, m.merchant_name AS name,  %#m.properties AS properties, "
  ...
  private static final String FUNDRAISER_SUMMARY =
      "SELECT m.merchant_id AS merchantId, m.merchant_name AS name,  %#m.properties AS properties, "
          + "(SELECT count(distinct deal_offer_purchase_id) FROM deal_offer_purchase WHERE properties->'merchant_code' IN "
          + "(SELECT mc.code FROM merchant_code AS mc, merchant_code_group AS mcg WHERE mcg.merchant_code_group_id = mc.merchant_code_group_id AND mcg.merchant_id = m.merchant_id) ) AS dealOffersSoldCount, "
          + "(SELECT count(distinct mc.code) FROM merchant_code AS mc, merchant_code_group AS mcg WHERE mcg.merchant_code_group_id = mc.merchant_code_group_id AND mcg.merchant_id = m.merchant_id) AS merchantCodeCount "
          + "FROM merchant AS m " + "WHERE m.is_discoverable = 't' ";
  ...
  private static final String PUBLISHER_CUSTOMER_SUMMARY =
      "select c.customer_id as customerId,c.email as email,c.first_name as firstName,c.last_name as lastName,"
          ...
          + "as commaSeperatedDealOfferTitles from customer as c " + "where c.customer_id in ("
  ...
  private static final String PUBLISHER_CUSTOMER_SUMMARY_CNT =
      "select count(distinct c.customer_id) as totalResults from customer as c, deal_offer_purchase as dop,deal_offer as dof "
          + "where dop.deal_offer_id=dof.deal_offer_id and dof.merchant_id=:publisherMerchantId and c.customer_id=dop.customer_id";
  ...
  private static final String PUBLISHER_CUSTOMER_EMAIL_SUMMARY_CNT =
      "select count(distinct c.customer_id) as totalResults from customer as c, deal_offer_purchase as dop,deal_offer as dof "
          + "where dop.deal_offer_id=dof.deal_offer_id and dof.merchant_id=:publisherMerchantId and c.customer_id=dop.customer_id and c.email like :email";
  ...
  private static final String ACTIVATION_SUMMARY =
      "select n1.doid as \"dealOfferId\",n1.title, n1.total as \"totalCodes\", n2.ta as \"totalActivations\" from  "
          ...
          + "right outer join " + "(select d.deal_offer_id as doid,d.title,count(*) as total from deal_offer d "
  ...
  public static final String DEAL_ACQUIRES = "select distinct dealAcquire from DealAcquireImpl as dealAcquire left join fetch dealAcquire.deal as d "
      ...
      + "where dealAcquire.deal.merchant.id=:merchantId and dealAcquire.customer.id=:customerId and dealAcquire.deal.expires > :expiresAfter";

  private static final String MERCHANT_ACQUIRES = "select distinct merchant from MerchantImpl merchant, DealAcquireImpl da, "
      + "DealImpl d left join fetch merchant.locations where da.customer.id=:customerId " + "and da.deal.id=d.id and d.merchant.id=merchant.id";
  ...
  private static final String MERCHANT_ACQUIRES_LOCATION =
      "select merchant.merchant_id as merchantId,merchant.merchant_name as merchantName,merchant.category_id as categoryId,"
          ...
          + "ST_Distance( location.geom,'${point}',true) as distanceInMeters " + "from "
  ...
  private static final String STOCK_MEDIA_WITHOUT_TAGS =
      "select distinct merchantMedia from MerchantMediaImpl as merchantMedia left join merchantMedia.tags t "
          + "where merchantMedia.merchantId=:merchantId and merchantMedia.mediaType in (:mediaTypes) " + "group by merchantMedia having count(t)=0";
  ...
  // DEALS_BY_DEAL_OFFER_ID will return dups, need to filter via
  // setResultTransformer(CriteriaSpecification.DISTINCT_ROOT_ENTITY);
  private static final String ACTIVE_DEALS_BY_DEAL_OFFER_ID =
      "select d from DealImpl as d left join fetch d.image left join fetch d.merchant as merchant "
          + "left join fetch merchant.locations where d.dealOffer.id=:dealOfferId and d.isActive=true and d.expires>:expiresDate";

  private static final String MERCHANTS_BY_DEAL_OFFER_ID =
      "select distinct merchant from MerchantImpl as merchant,DealImpl as dof where dof.dealOffer.id=:dealOfferId and dof.merchant.id=merchant.id";
  ...
  private static final String DEAL_SUMMARY_BY_DEAL_ID =
      "SELECT d.deal_id AS dealId, d.deal_offer_id AS offerId, d.merchant_id AS merchantId, d.title AS title, d.summary AS summary, d.details AS details, "
          + "d.expires AS expires, d.is_active AS isActive, l.city AS merchantCity, l.state_province_county AS merchantState, "
          + "(SELECT array_to_string(array(SELECT DISTINCT t.name FROM tag AS t, deal_tag AS dt WHERE dt.deal_id = d.deal_id AND dt.tag_id = t.tag_id),', ')) AS tags, "
          + "(SELECT mm.media_url FROM merchant_media AS mm WHERE mm.merchant_media_id = d.image_id) AS imageUrl,"
          ...
          + "JOIN (SELECT merchant_location_id, merchant_id, city, state_province_county, create_dt FROM merchant_location ml1 "
          + "WHERE (merchant_location_id, create_dt) IN (SELECT merchant_location_id, create_dt FROM merchant_location ml2 WHERE ml1.merchant_id = ml2.merchant_id ORDER BY create_dt LIMIT 1)) l "
          + "ON (d.merchant_id = l.merchant_id) " + "WHERE d.deal_id = :dealId";
  ...
  private static final String MOVE_DEALS = "UPDATE deal " + "SET deal_offer_id = :dealOfferId, updated_by_merchant_account_id = :merchantAccountId "
      + "WHERE deal_id IN (:dealIds)";

  private static final String RECIPIENT_STATUSES =
      "from RecipientStatusImpl as recipientStatus left join fetch recipientStatus.customer where recipientStatus.messagingJobId=:messagingJobId";
  ...
  public enum QueryType {
    ...
    ActivePaidAndFreeDealOfferIDsWithinMeters(ACTIVE_PAID_AND_FREE_DEAL_OFFER_IDS_WITHIN_METERS, EMPTY_IMMUTABLE_PROPS),
    ...
    MerchantsByDealOfferId(MERCHANTS_BY_DEAL_OFFER_ID, ImmutableMap.<String, String>builder().put("merchant.name", "merchant.name").build()),
    ...
    MerchantAcquiresLocation(MERCHANT_ACQUIRES_LOCATION, ImmutableMap.<String, String>builder().put("merchant.name", "merchant.merchant_name")
        .put("merchant.isDiscoverable", "isDiscoverable").put("merchant.locations.distanceInMeters", "distanceInMeters").build()),
    ...
    FavoriteMerchants(FAVORITE_MERCHANTS, ImmutableMap.<String, String>builder().put("merchant.created", "merchant.createdUpdated.created")
    ...
    DealOfferSummary(DEAL_OFFER_SUMMARY, EMPTY_IMMUTABLE_PROPS), DealOfferSummaryCnt(DEAL_OFFER_SUMMARY_CNT, EMPTY_IMMUTABLE_PROPS), DealOfferTitleSummaryCnt(
        DEAL_OFFER_TITLE_SUMMARY_CNT, EMPTY_IMMUTABLE_PROPS), PublisherDealOfferSummaryCnt(PUBLISHER_DEAL_OFFER_SUMMARY_CNT, EMPTY_IMMUTABLE_PROPS), PublisherDealOfferTitleSummaryCnt(
        PUBLISHER_DEAL_OFFER_TITLE_SUMMARY_CNT, EMPTY_IMMUTABLE_PROPS), PublisherDealOfferSummary(PUBLISHER_DEAL_OFFER_SUMMARY, EMPTY_IMMUTABLE_PROPS), DealOfferTitleSummary(
        DEAL_OFFER_TITLE_SUMMARY, EMPTY_IMMUTABLE_PROPS), PublisherDealOfferTitleSummary(PUBLISHER_DEAL_OFFER_TITLE_SUMMARY, EMPTY_IMMUTABLE_PROPS),

    MerchantSummaryCnt(MERCHANT_SUMMARY_CNT, EMPTY_IMMUTABLE_PROPS), MerchantNameSummaryCnt(MERCHANT_NAME_SUMMARY_CNT, EMPTY_IMMUTABLE_PROPS), PublisherMerchantSummaryCnt(
        PUBLISHER_MERCHANT_SUMMARY_CNT, EMPTY_IMMUTABLE_PROPS), PublisherMerchantNameSummaryCnt(PUBLISHER_MERCHANT_NAME_SUMMARY_CNT,
        EMPTY_IMMUTABLE_PROPS), MerchantSummary(MERCHANT_SUMMARY, EMPTY_IMMUTABLE_PROPS), MerchantNameSummary(MERCHANT_NAME_SUMMARY,
        EMPTY_IMMUTABLE_PROPS), PublisherMerchantSummary(PUBLISHER_MERCHANT_SUMMARY, EMPTY_IMMUTABLE_PROPS), PublisherMerchantNameSummary(
        PUBLISHER_MERCHANT_NAME_SUMMARY, EMPTY_IMMUTABLE_PROPS), PublisherFundraiserSummary(PUBLISHER_FUNDRAISER_SUMMARY, EMPTY_IMMUTABLE_PROPS), FundraiserSummary(
        FUNDRAISER_SUMMARY, EMPTY_IMMUTABLE_PROPS),

    MerchantCodeSummaryCnt(MERCHANT_CODE_SUMMARY_CNT, EMPTY_IMMUTABLE_PROPS), MerchantCodeSummary(MERCHANT_CODE_SUMMARY, EMPTY_IMMUTABLE_PROPS),

    DealSummary(DEAL_SUMMARY, EMPTY_IMMUTABLE_PROPS), DealSummaryCnt(DEAL_SUMMARY_CNT, EMPTY_IMMUTABLE_PROPS), DealSummaryByDealId(
        DEAL_SUMMARY_BY_DEAL_ID, EMPTY_IMMUTABLE_PROPS),
   ...
   * @param params
   * @param searchOpts
   * @param ignoreLimitOffset - HQL seems to throw away any LIMIT/OFFSET set, so we can optionally
   *        ignore for HQL
   * @return
   */
  ...
  public static String buildPropertyQuery(final QueryType queryType, final PropertyCriteria propertyCriteria, final SearchOptions searchOpts) {
