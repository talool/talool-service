...
public final class ActivityFactory
{
	...
	public static Activity createPurchase(final DealOffer dealOffer, final UUID customerUuid) throws TException
	{
		...
		final String title = BundleUtil.render(BundleType.ACTIVITY, Locale.ENGLISH, ActivityBundle.PURCHASED_DEAL_OFFER_TITLE,
				dealOffer.getTitle());
	...
	public static Activity createFundraiserSupport(final DealOfferPurchase dop, final Merchant fundraiser) throws TException
	{
		...
		String title = BundleUtil.render(BundleType.ACTIVITY, Locale.ENGLISH, ActivityBundle.FUNDRAISER_SUPPORT_TITLE,
				fundraiser.getName());
			...
			throws TException
	{
		...
		final String title = BundleUtil.render(BundleType.ACTIVITY, Locale.ENGLISH, ActivityBundle.ACTIVATED_DEAL_OFFER_TITLE,
				dealOffer.getTitle(), code.toUpperCase());
	...
	{
		final Activity activity = domainFactory.newActivity(ActivityEvent.FRIEND_GIFT_REDEEM, dealAcquire.getGift().getFromCustomer()
				.getId());
		final Activity_t tActivity = createBaseActivity_t(ActivityEvent_t.FRIEND_GIFT_REDEEM);

		String title = BundleUtil.render(BundleType.ACTIVITY, Locale.ENGLISH, ActivityBundle.FRIEND_REDEEMED_DEAL_TITLE, dealAcquire
				.getCustomer().getFullName());

		tActivity.setTitle(title);

		title = BundleUtil.render(BundleType.ACTIVITY, Locale.ENGLISH, ActivityBundle.FRIEND_REDEEMED_DEAL_SUBTITLE, dealAcquire.getDeal()
				.getTitle(), dealAcquire.getDeal().getMerchant().getName());
	...
	public static Activity createFriendRejectGift(final Gift gift) throws TException
	{
		...
		String title = BundleUtil.render(BundleType.ACTIVITY, Locale.ENGLISH, ActivityBundle.FRIEND_REJECTED_DEAL_TITLE,
				gift.getReceipientName());

		tActivity.setTitle(title);

		title = BundleUtil.render(BundleType.ACTIVITY, Locale.ENGLISH, ActivityBundle.FRIEND_REJECTED_DEAL_SUBTITLE, gift.getDealAcquire()
				.getDeal().getTitle(), gift.getDealAcquire().getDeal().getMerchant().getName());
	...
	public static Activity createGiftReturnNotExistentEmail(final Gift gift) throws TException
	{
		...
		String title = BundleUtil.render(BundleType.ACTIVITY, Locale.ENGLISH, ActivityBundle.GIFT_RETURNED_EMAIL_DOES_NOT_EXIST_TITLE,
				gift.getDealAcquire().getDeal().getTitle());

		tActivity.setTitle(title);

		title = BundleUtil.render(BundleType.ACTIVITY, Locale.ENGLISH, ActivityBundle.GIFT_RETURNED_EMAIL_DOES_NOT_EXIST_SUBTITLE,
	...
	public static Activity createFriendAcceptGift(final Gift gift) throws TException
	{
		...
		String title = BundleUtil.render(BundleType.ACTIVITY, Locale.ENGLISH, ActivityBundle.FRIEND_ACCEPTED_DEAL_TITLE,
				gift.getReceipientName());

		tActivity.setTitle(title);

		title = BundleUtil.render(BundleType.ACTIVITY, Locale.ENGLISH, ActivityBundle.FRIEND_ACCEPTED_DEAL_SUBTITLE, gift.getDealAcquire()
				.getDeal().getTitle(), gift.getDealAcquire().getDeal().getMerchant().getName());
	...
	public static Activity createReject(final Gift gift, UUID customerUuid) throws TException
	{
		...
		String title = BundleUtil.render(BundleType.ACTIVITY, Locale.ENGLISH, ActivityBundle.REJECTED_GIFT_TITLE, gift.getDealAcquire()
				.getDeal().getTitle(), gift.getDealAcquire().getDeal().getMerchant().getName());

		tActivity.setTitle(title);

		title = BundleUtil.render(BundleType.ACTIVITY, Locale.ENGLISH, ActivityBundle.REJECTED_GIFT_SUBTITLE, gift.getFromCustomer()
				.getFullName());
	...
	public static Activity createEmailSendGift(final Gift gift) throws TException
	{
		...
		String title = BundleUtil.render(BundleType.ACTIVITY, Locale.ENGLISH, ActivityBundle.SENT_EMAIL_GIFT_TITLE, emailGift
				.getDealAcquire().getDeal().getTitle(), emailGift.getDealAcquire().getDeal().getMerchant().getName());

		tActivity.setTitle(title);

		title = BundleUtil.render(BundleType.ACTIVITY, Locale.ENGLISH, ActivityBundle.SENT_EMAIL_GIFT_SUBTITLE,
				emailGift.getReceipientName(), emailGift.getToEmail());
	...
	public static Activity createFacebookSendGift(final Gift gift) throws TException
	{
		...
		String title = BundleUtil.render(BundleType.ACTIVITY, Locale.ENGLISH, ActivityBundle.SENT_FACEBOOK_GIFT_TITLE, gift
				.getDealAcquire().getDeal().getTitle(), gift.getDealAcquire().getDeal().getMerchant().getName());
	...
	public static Activity createFacebookRecvGift(final Gift gift) throws TException
	{
		...
		String title = BundleUtil.render(BundleType.ACTIVITY, Locale.ENGLISH, ActivityBundle.RECV_FACEBOOK_GIFT_TITLE, gift
				.getDealAcquire().getDeal().getTitle(), gift.getDealAcquire().getDeal().getMerchant().getName());

		tActivity.setTitle(title);

		title = BundleUtil.render(BundleType.ACTIVITY, Locale.ENGLISH, ActivityBundle.RECV_FACEBOOK_GIFT_SUBTITLE, gift.getFromCustomer()
				.getFullName());
	...
	public static Activity createEmailRecvGift(final Gift gift) throws TException
	{
		...
		String title = BundleUtil.render(BundleType.ACTIVITY, Locale.ENGLISH, ActivityBundle.RECV_EMAIL_GIFT_TITLE, gift.getDealAcquire()
				.getDeal().getTitle(), gift.getDealAcquire().getDeal().getMerchant().getName());

		tActivity.setTitle(title);

		title = BundleUtil.render(BundleType.ACTIVITY, Locale.ENGLISH, ActivityBundle.RECV_EMAIL_GIFT_SUBTITLE, gift.getFromCustomer()
				.getFullName());
	...
	public static Activity createRedeem(final DealAcquire dealAcquire, final UUID customerUuid) throws TException
	{
		...
		String title = BundleUtil.render(BundleType.ACTIVITY, Locale.ENGLISH, ActivityBundle.REDEEMED_DEAL_TITLE, dealAcquire.getDeal()
				.getTitle(), dealAcquire.getDeal().getMerchant().getName());
		...
		{
			title = BundleUtil.render(BundleType.ACTIVITY, Locale.ENGLISH, ActivityBundle.REDEEMED_DEAL_SUBTITLE, dealAcquire.getGift()
