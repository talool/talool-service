...
public interface CustomerService extends HibernateService, RequestHeaderSupport {
  ...
  /**
   * Creates a customer account with the given password. If the whiteLabelPublisherMerchantId is
   * null it is ignored, otherwise it is associated to the customer account.
   * 
   * @param customer
   * @param password
   * @param whiteLabelPublisherMerchantId (can be null)
  ...
  public List<DealAcquire> getDealAcquires(final UUID customerId, final UUID merchantId, final SearchOptions searchOptions) throws ServiceException;
  ...
  /**
   * Redeems the deal if all the acquireStatus is in the proper state. Returns a confirmation code
  ...
  /**
   * Gets merchants acquired by customerId using searchOptions. If location is not null, the
   * distanceInMeters is calculated for every merchant location
   * 
   * @param customerId
   * @param categoryId
   * @param searchOptions
  ...
  /**
   * Creates a password reset code and a password reset expires. These are saved on the customer
   * (getPasswordResetCode, getPasswordResetExpires)
   * 
   * @param customer
   * @throws ServiceException
   */
  ...
  /**
   * Gets all the customers of a publisher. A publishers customers are defined by having at least 1
   * deal offer belonging to the publisher
   * 
