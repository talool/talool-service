...
public interface MerchantService extends HibernateService {
  ...
  /**
   * For now it will throw an error if there are more than 1 MerchantAccounts for the given email .
   * In order to support multiple accounts, do a separate call to get the MerchantAccounts for the
   * email
   * 
   * @param email
   * @param password
  ...
  /**
   * Gets all deals by deal offer. If activeDealsOnly=true, expired or inActive deals will not be
   * returned.
   * 
   * @param dealOfferId
   * @param searchOpts
  ...
  public List<Merchant> getMerchantsWithin(final Location location, final int maxMiles, final SearchOptions searchOptions) throws ServiceException;

  public List<Merchant> getMerchantsWithin2(final Location location, final int maxMiles, final SearchOptions searchOptions) throws ServiceException;
  ...
  /**
   * Gets all the offers of a publisher. A publishers offers are defined by a merchant that is the
   * publisher
   * 
   ...
   * @throws ServiceException
   */
  public PaginatedResult<DealOfferSummary> getPublisherDealOfferSummary(final UUID publisherMerchantId, final SearchOptions searchOpts,
      final boolean calculateRowSize) throws ServiceException;

  public PaginatedResult<DealOfferSummary> getPublisherDealOfferSummaryByTitle(final UUID publisherMerchantId, final SearchOptions searchOpts,
      final String title, final boolean calculateRowSize) throws ServiceException;
  ...
  /**
   * Gets all the merchants of a publisher. A publishers merchants are defined by having a location
  ...
  public PaginatedResult<MerchantSummary> getPublisherMerchantSummaryByName(final UUID publisherMerchantId, final SearchOptions searchOpts,
      final String name, final PropertyCriteria propertyCriteria, final boolean calculateRowSize) throws ServiceException;

  public long getPublisherMerchantSummaryNameCount(final UUID publisherMerchantId, final String name, final PropertyCriteria propertyCriteria)
      throws ServiceException;
  ...
  public long getPublisherFundraiserSummaryCount(final UUID publisherMerchantId) throws ServiceException;
  ...
  /**
   * Deletes the deal if it has no DealAcquires. Otherwise, it moves the deal to a Kirke book for
   * the publisher and throws a service exception
   * 
   * @param dealId
