...
public class TaloolServiceTest extends HibernateFunctionalTestBase {
  ...
  public void testMerchantCodes() throws ServiceException {
    ...
    MerchantCodeGroup merchantCodeGrp =
        taloolService.createMerchantCodeGroup(school, merchantAccount.getId(), publisher.getId(), "Lewis Palmer School codes", "first set of notes",
            (short) 100);
  ...
  public void testMedia() throws ServiceException, InterruptedException {
    ...
    SearchOptions searchOptions = new SearchOptions.Builder().maxResults(100).page(0).sortProperty("merchantMedia.mediaUrl").ascending(true).build();
   ...
   */
  public void testDealAquireSearchOptions(DealOfferPurchase dealOfferPurchase, DealOffer dealOffer, List<DealAcquire> expectedDealAcquires)
      throws ServiceException {
    ...
    for (int sortTypeIndex = 0; sortTypeIndex <= 1; sortTypeIndex++) {
      ...
      while (true) {
        SearchOptions searchOpts =
            new SearchOptions.Builder().maxResults(maxResults).page(page++).sortProperty("dealAcquire.deal.title").ascending(ascending).build();

        dealAcquires =
            customerService.getDealAcquires(dealOfferPurchase.getCustomer().getId(), dealOfferPurchase.getDealOffer().getMerchant().getId(),
                searchOpts);
  ...
  public void testDealAquireMerchants(DealOfferPurchase dealOfferPurchase, DealOffer dealOffer, List<DealAcquire> expectedDealAcquires)
    ...
    for (int sortTypeIndex = 0; sortTypeIndex <= 1; sortTypeIndex++) {
      ...
      while (true) {
        SearchOptions searchOpts =
            new SearchOptions.Builder().maxResults(maxResults).page(page++).sortProperty("merchant.name").ascending(ascending).build();
  ...
  public void testGetGiftsForCustomer() throws ServiceException {
    List<Gift> gifts = customerService.getGifts(UUID.fromString("d5959550-2fee-4612-9daf-9d6df2347f62"), new GiftStatus[] {GiftStatus.PENDING});
  ...
  public void testGiftRequests() throws ServiceException {
    ...
    // give 2nd via email
    customerService.giftToEmail(givingCustomer.getId(), giftedDealAcquireViaEmail.getId(), receivingCustomerEmail.getEmail(), receivingName);
    ...
    for (Gift gf : gifts) {
      if (gf instanceof FaceBookGift) {
        ...
        Assert.assertEquals(receivingCustomerSocial.getSocialAccounts().get(taloolService.getSocialNetwork(SocialNetwork.NetworkName.Facebook))
  ...
  public DealOffer testDealOffers(final Merchant merchant, final MerchantAccount merchantAccount, final Customer customer) throws ServiceException {
  ...
  public MerchantAccount testMerchantAccount(Merchant merchant) throws ServiceException, NoSuchAlgorithmException, UnsupportedEncodingException {
  ...
  public Merchant testCreateMerchant() throws ServiceException {
    ...
    Assert.assertEquals(merchant.getPrimaryLocation().getStateProvinceCounty(), resultMerch.getPrimaryLocation().getStateProvinceCounty());
  ...
  public void testFavoriteMerchants() throws ServiceException {
    ...
    SearchOptions searchOptions = new SearchOptions.Builder().maxResults(100).page(0).sortProperty("merchant.created").ascending(true).build();
  ...
  public void testCustomerSummary() throws ServiceException, InvalidInputException {
    SearchOptions searchOpts = new SearchOptions.Builder().sortProperty("redemptions").ascending(false).maxResults(100).page(0).build();
